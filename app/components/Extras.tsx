...
export default function Extras() {
  ...
  return (
    <>
      {/* Currently Learning */}
      <section ref={r2} style={{ padding: "5rem 2rem", background: "rgba(13,27,46,0.4)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={i2 ? { opacity: 1, y: 0 } : {}}>
            ...
            <p style={{ color: "#64748b", marginBottom: "2.25rem", fontSize: "0.9rem" }}>Always building. Always learning.</p>
          </motion.div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "0.9rem" }}>
            {learning.map((l, i) => (
              <motion.div key={l.label}
                initial={{ opacity: 0, scale: 0.95 }}
                animate={i2 ? { opacity: 1, scale: 1 } : {}}
                transition={{ delay: i * 0.07 }}
                style={{ display: "flex", gap: "0.9rem", padding: "1.1rem 1.25rem", background: "rgba(13,27,46,0.8)", border: "1px solid rgba(59,130,246,0.1)", borderRadius: "12px", alignItems: "flex-start" }}
              >
                ...
                <div>
                  ...
                  <div style={{ color: "#64748b", fontSize: "0.78rem", lineHeight: 1.5 }}>{l.desc}</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* Principles */}
      <section ref={r1} style={{ padding: "5rem 2rem" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={i1 ? { opacity: 1, y: 0 } : {}}>
            <span className="section-label">Philosophy</span>
            <h2 style={{ fontSize: "clamp(1.8rem, 3.5vw, 2.6rem)", fontWeight: 800, marginTop: "0.5rem", marginBottom: "2.25rem", letterSpacing: "-0.02em" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.9rem" }}>
            {principles.map((p, i) => (
              <motion.div key={p.num}
                ...
                animate={i1 ? { opacity: 1, y: 0 } : {}}
                transition={{ delay: i * 0.07 }}
                style={{ padding: "1.25rem 1.4rem", background: "rgba(13,27,46,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "12px" }}
                onMouseEnter={e => (e.currentTarget.style.borderColor = "rgba(6,182,212,0.25)")}
                onMouseLeave={e => (e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)")}
              >
                <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "0.68rem", color: "#06B6D4", marginBottom: "0.4rem" }}>{p.num}</div>
