...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} style={{ padding: "4rem 1.25rem", background: "rgba(13,27,46,0.4)" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        {/* Stats grid — always 2x2, scales up */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.15 }}
          style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "2.5rem", maxWidth: "460px" }}
        >
          {stats.map((s) => (
            <div key={s.label} style={{ textAlign: "center", padding: "1rem 0.75rem", background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: "12px" }}>
              <div style={{ fontSize: "clamp(1.5rem, 4vw, 2rem)", fontWeight: 800, background: "linear-gradient(135deg,#3B82F6,#06B6D4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{s.value}</div>
              <div style={{ color: "#64748b", fontSize: "0.72rem", marginTop: "0.2rem", fontFamily: "JetBrains Mono, monospace" }}>{s.label}</div>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ delay: 0.2 }}
          ...
          {[
            ...
            <p key={i} style={{ color: "#94a3b8", lineHeight: 1.85, fontSize: "clamp(0.875rem, 1.8vw, 0.95rem)", marginBottom: "1.1rem" }}>{para}</p>
          ))}
          <p style={{ color: "#94a3b8", lineHeight: 1.85, fontSize: "clamp(0.875rem, 1.8vw, 0.95rem)" }}>
